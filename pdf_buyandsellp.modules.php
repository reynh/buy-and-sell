 ...
 */
class pdf_buyandsellp extends ModelePDFPropales
{
	/**
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		if ($conf->propal->multidir_output[$conf->entity])
		{
			...
			if (file_exists($dir))
			{
				...
				$tab_top_newpage = (empty($conf->global->MAIN_PDF_DONOTREPEAT_HEAD)?48+$top_shift:10);
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					//	$pdf->MultiCell($this->posxup - $this->posxtva + 8, 3, $outputlangs->convToOutputCharset($product->array_options['options_taxable']), 0, 'C');
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if (empty($conf->global->PROPALE_PDF_HIDE_PAYMENTTERMMODE))
		{
			...
			*/
			...
			$pdf->SetXY($posx+65 , $posy+07);
			$titre = $outputlangs->transnoentities("Thank you kindly for reviewing our proposal!");
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='')
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxqty-$this->posxup+10,10, $outputlangs->transnoentities("PriceUHT"),'','C');
		}
		...
		if($conf->global->PRODUCT_USE_UNITS) {
			...
			if (empty($hidetop)) {
				...
				$pdf->MultiCell($this->posxdiscount - $this->posxunit - 1, 2, "U/M", '',
					'C');
			}
		}
...
", 0, 'L');
			...
			// Show recipient
			$widthrecbox = !empty($conf->global->MAIN_PDF_USE_ISO_LOCATION) ? 92 : 55;
			if ($this->page_largeur < 210) $widthrecbox = 84; // To work with US executive format
			$posy = !empty($conf->global->MAIN_PDF_USE_ISO_LOCATION) ? 40 : 57;
			$posy += $top_shift;
			$posx = $this->page_largeur - $this->marge_droite - $widthrecbox -76;
			...
			$pdf->writeHTMLCell (30,3, $this->posxdesc+66, $tab_top+60, $outputlangs->convToOutputCharset($object->array_options['options_jobaddress']),0,1);
			...
			//$pdf->MultiCell($widthrecbox, 4, $carac_client, 0, 'L');
								             ...
								             // Show recipient  3
			$widthrecbox = !empty($conf->global->MAIN_PDF_USE_ISO_LOCATION) ? 92 : 67;
			if ($this->page_largeur < 210) $widthrecbox = 84; // To work with US executive format
			$posy = !empty($conf->global->MAIN_PDF_USE_ISO_LOCATION) ? 40 : 57;
			$posy += $top_shift;
			$posx = $this->page_largeur - $this->marge_droite - $widthrecbox -6;
			if (!empty($conf->global->MAIN_INVERT_SENDER_RECIPIENT)) $posx = $this->marge_gauche;
			...
			$pdf->SetFont('', '', 9);
			$pdf->MultiCell($w, 3, $outputlangs->transnoentities("Date")." : ".dol_print_date($object->date, "day", false, $outputlangs,true), 0, 'L');
			$pdf->SetXY($posx + 2, $posy + 5);
			$pdf->SetFont('', '', 9);
			$pdf->MultiCell($w, 3, $outputlangs->transnoentities("DateEndPropal")." : ".dol_print_date($object->fin_validite, "day", false, $outputlangs, true), 0, 'L');
			...
			$pdf->SetFont('', '', 9);
			/*$pdf->MultiCell($w, 3, "Cust Pick Up"." : ", 0, 'L');
						   $pdf->SetXY($posx + 2, $posy + 16);
			$pdf->SetFont('', '', 9);
			$pdf->MultiCell($w, 3, "Sales Executive"." : ", 0, 'L');*/
			$posy = pdf_writeLinkedObjects($pdf, $object, $outputlangs, $posx+2, $posy+9.5, $w, 33, 'L', 11);
				$extrafields = new ExtraFields($this->db);
$extralabels=$extrafields->fetch_name_optionals_label($object->table_element);
$object->fetch($rowid);
$object->fetch_optionals($rowid,$extralabels);
$pdf->SetFont('', '', 9);
$pdf->writeHTMLCell (190,3, $this->posxdesc+160, $tab_top-202, $extrafields->showOutputField('salesexecutive', $object->array_options['options_salesexecutive']),0,1);
$pdf->writeHTMLCell (190,3, $this->posxdesc+160, $tab_top-206, $extrafields->showOutputField('customerpickup', $object->array_options['options_customerpickup']),0,1);
 //$pdf->writeHTMLCell(190, 3, $this->posxdesc+50, $tab_top-198, "Modified Date: date('Y-m-d H:i:s'){STYLE font-size: 10; text-align: right; font-style: italic;}", 0, 1);
	...
	function _signature_area(&$pdf, $object, $posy, $outputlangs)
	{
		...
		$pdf->write1DBarcode($outputlangs->transnoentities($object->thirdparty->code_client), 'C128', '', '', '', 18, 0.4, $style, 'N');
