...
class pdf_einstein extends ModelePDFCommandes
{
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->commande->multidir_output[$conf->entity]) {
           ...
           $sql5=  "SELECT  CASE(pex.taxable) WHEN '1' THEN 'Y' WHEN '0' THEN 'N' END AS tax FROM ".MAIN_DB_PREFIX."commandedet AS f ";
                    ...
                    $sql5.= " WHERE  f.fk_commande =  ".$object->id;
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN)) {
						...
						//$pdf->MultiCell($this->posxup - $this->posxtva - 1, 3, $hid[$i][0], 0, 'R');
					}
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis = null)
	{
		...
		$pdf->SetXY($col1x+20, $tab2_top + 0);
		$pdf->MultiCell($col2x - $col1x, $tab2_hl, 'Amount', 0, 'L', 1);
				...
				foreach ($this->tva_array as $tvakey => $tvaval) {
					if ($tvakey != 0) {    // On affiche pas taux 0
						...
						if (getDolGlobalString('PDF_VAT_LABEL_IS_CODE_OR_RATE') == 'rateonly') {
							...
							$totalvat .= number_format($tvaval['vatrate'],2,".",",").($tvaval['vatcode'] ? ' ('.$tvaval['vatcode'].')' : '').$tvacompl."%";
						}
$pdf->SetXY($col2x-30, $tab2_top + $tab2_hl * $index);
						$pdf->MultiCell($col2x - $col1x, $tab2_hl, 'Sales Tax', 0, 'L', 0);

$pdf->SetXY($col2x-56, $tab2_top + $tab2_hl * $index);
				...
				// Total TTC
				$index++;
				$pdf->SetXY($col1x+20, $tab2_top + $tab2_hl * $index);
				$pdf->SetTextColor(0, 0, 60);
				$pdf->SetFillColor(224, 224, 224);
$pdf->SetFont('', 'B', $default_font_size - 1);
				$pdf->MultiCell($col2x - $col1x, $tab2_hl, 'Please Pay This Amount', $useborder, 'L', 1);
   ...
   $sql=  " SELECT p.jobaddress AS nu FROM "  . MAIN_DB_PREFIX . "commande_extrafields AS p ";
...
Jobaddress: " . $dat3, 0, $ltrdirection);
//$pdf->MultiCell($widthrecbox, 4,"Jobaddress: " . $dat3, 0, $ltrdirection);
		}
